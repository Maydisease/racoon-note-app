...
class CategoryContainer extends React.Component {
	...
	constructor(readonly props: any) {
		...
		this.goToLinksModule = this.goToLinksModule.bind(this);
	}
	...
	public goToLinksModule() {
		history.push('/book_mark');
	}
			...
			<div className={`categoryContainer ${(this.state.componentShowState && !STORE_NOTE$FRAME.linkMode) ? 'show' : ''}`}>
                ...
                <span className={`icon icon-2`}>
					...
					<div className="action-bar">
						...
						<span className="line left"/>
						<label className={`left`} onClick={this.handleActionBar.bind(null, 'links')}>
							<FontAwesomeIcon className="fa-icon" icon="link"/>
						</label>
						<span className="line left"/>
